...
const INPUT_CLS =
  "w-full border-[1.5px] border-border rounded-lg px-4 py-3 text-[14px] font-sans text-tx bg-surface outline-none focus:border-navy-800 focus:shadow-[0_0_0_3px_rgba(29,49,96,.08)] transition-all";

function LoginEmblem() {
  return (
    <div
      className="w-15 h-15 rounded-full bg-navy-950 border-2 border-gold-600/60 flex items-center justify-center mx-auto mb-4"
      style={{
        boxShadow: "0 0 0 5px rgba(192,144,32,.1), 0 8px 24px rgba(0,0,0,.4)",
      }}
    >
      <svg width="36" height="36" viewBox="0 0 54 54">
        ...
        <path
          d="M10 33 Q18 29 27 33 Q36 37 44 31"
          stroke="#60A5FA"
          strokeWidth="2.5"
          fill="none"
          strokeLinecap="round"
    ...
    <div>
      <Field label="Email Address" labelMr="ईमेल पत्ता">
        <input
          ...
          type="email"
          inputMode="email"
          placeholder="officer@water.maharashtra.gov.in"
          value={email}
      ...
      >
        🔑 {mr ? "लॉगिन करा" : "Login"}{" "}
        <span className="dv text-[12px] opacity-80">लॉगिन करा</span>
      </Button>
  ...
  return (
    <div className="py-2">
      <InfoBox type="info" icon="🏛️">
        {mr
          ? "अधिकारी नोंदणी प्रणाली व्यवस्थापकाद्वारे केली जाते."
          : "Officer accounts are created by your system administrator."}
        <span className="dv block text-[12px] mt-1">
          {mr
            ? "स्व-नोंदणी उपलब्ध नाही."
      ...
      <div className="mt-5 rounded-xl border border-border-2 bg-surface-2 px-5 py-4 space-y-2.5">
        ...
        <ol className="text-[12.5px] text-muted space-y-1.5 pl-4 list-decimal">
          <li>
            {mr
              ? "आपल्या विभागीय सुपर प्रशासकाशी संपर्क साधा."
              : "Contact your divisional Super Admin."}
          </li>
          <li>
            {mr
              ? "ते तुमचे ईमेल व भूमिका नोंदवतील."
              : "They will register your email and role."}
          </li>
          <li>
            {mr
              ? "नोंदणीनंतर Login टॅबवर ईमेल OTP वापरा."
              : "Once registered, use the Login tab with your email OTP."}
          </li>
        </ol>
      </div>
    </div>
  ...
  return (
    <div className="mb-5">
      <label className="block text-[13px] font-semibold text-navy-950 mb-2 tracking-[.1px]">
        {label}
        {labelMr && (
          <span className="dv text-[12px] text-muted font-normal block mt-px">
            {labelMr}
          </span>
        )}
      </label>
  ...
  return (
    <div className="mt-4 w-full max-w-120 rounded-xl border border-amber-300/60 overflow-hidden">
      <div className="px-4 py-2 bg-amber-50 border-b border-amber-300/60 flex items-center gap-2">
        <span className="text-[10.5px] font-bold uppercase tracking-[.8px] text-amber-700">
          ⚙ Dev Mode
        </span>
        <span className="text-[10.5px] text-amber-600/60">
          · bypasses OTP · dev builds only
        </span>
      </div>
      <div className="px-4 py-3 bg-white/70 flex flex-col gap-2">
        {groups.map((g) => (
          <div key={g.label} className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] font-bold uppercase tracking-[.6px] text-amber-600/70 w-16 shrink-0">
              {g.label}
            </span>
            {g.entries.map((e) => (
              <button
...
export default function Login() {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(155deg, #E2EAF5 0%, #ECF1FA 50%, #E6EDF7 100%)",
      }}
    >
      <div
        className="bg-surface w-full max-w-120 rounded-2xl overflow-hidden animate-fade-up"
        style={{ boxShadow: "var(--shadow-login)" }}
      >
        {/* Card header / banner */}
        <div
          className="relative px-9 py-7 text-center overflow-hidden"
          style={{
            background:
              "linear-gradient(170deg, #081629 0%, #142244 55%, #1A3060 100%)",
          }}
        >
          ...
          <div
            ...
            style={{
              backgroundImage:
                "repeating-linear-gradient(135deg, rgba(255,255,255,.02) 0px, rgba(255,255,255,.02) 1px, transparent 1px, transparent 10px)",
            }}
          />
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, transparent 0%, #C09020 25%, #E8B84B 50%, #C09020 75%, transparent 100%)",
            }}
          />
          ...
          <div className="dv text-[12.5px] text-white/45 mt-2">
            {t("loginSub", lang)}
          </div>

          {/* Tabs */}
          <div
            className="flex mt-7 rounded-xl p-1 gap-1"
